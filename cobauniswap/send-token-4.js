...
const valueInEther = '0.21';

async function sendTokens() {
  const contract = new web3.eth.Contract(abi, contractAddress);
  const nonce = await web3.eth.getTransactionCount(senderAddress);
  const value = web3.utils.toWei(String(valueInEther), 'ether');

  const data = contract.methods.transferFrom(senderAddress, receiverAddress, value).encodeABI();
