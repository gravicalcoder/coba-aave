...
//const contractABI = [{"inputs":[{"internalType":"address","name":"","type":"address"},{"internalType":"address","name":"onBehalfOf","type":"address"},{"internalType":"uint16","name":"referralCode","type":"uint16"}],"name":"depositETH","outputs":[],"stateMutability":"payable","type":"function"}];
const contractABI =  [
    {
      "inputs": [
        {
...
const privateKey = Buffer.from('0xec726be660698d6d06fda1b8ad2229886881c5fcc5a36109213b3c63d1cf5560', 'hex');

async function deposit() {
  const amountToDeposit = web3.utils.toWei('1', 'ether');
  const nonce = await web3.eth.getTransactionCount(senderAddress);

  const txData = contract.methods.depositETH(amountToDeposit, senderAddress, senderAddress, 0).encodeABI();
  const tx = {
    to: contractAddress,
    nonce: nonce,
