...
const poolAddress = '0x88f3C15523544835fF6c738DDb30995339AD57d6'

async function getSqrtPriceLimitX96(poolAddress,poolAbi ) {
  ...
  //const USDC_ABI = [{"inputs":[{"internalType":"address","name":"spender","type":"address"},{"internalType":"uint256","name":"amount","type":"uint256"}],"name":"approve","outputs":[{"internalType":"bool","name":"","type":"bool"}],"stateMutability":"nonpayable","type":"function"}];
  ...
  //const USDC_AMOUNT = '1000000000000000000'; // amount of USDC tokens to approve, in wei
  const USDC_AMOUNT = '2000000000000000000'; // amount of USDC tokens to approve, in wei
  
  async function approveUSDC() {
    //V3SwapRouterABI  
    //ROUTER_ADDRESS
    //const usdcContract = new web3.eth.Contract(USDC_ABI, USDC_CONTRACT_ADDRESS);
    //const usdcContract = await new web3.eth.Contract(V3SwapRouterABI, USDC_CONTRACT_ADDRESS);
    const usdcContract = await new web3.eth.Contract(USDC_ABI, ROUTER_ADDRESS);


    const txData = await usdcContract.methods.approve(ROUTER_ADDRESS, USDC_AMOUNT).encodeABI();
    //const txData = await usdcContract.methods.approve(USDC_CONTRACT_ADDRESS, USDC_AMOUNT).encodeABI();
    const nonce = await web3.eth.getTransactionCount(WALLET_ADDRESS);
    const gasPrice = await web3.eth.getGasPrice();
    const gasLimit = 48800;
    const tx = {
    ...
    console.log('USDC approved:', receipt);
  }
...
async function swap() {
  ...
  const sqrtPriceX96 = await getSqrtPriceLimitX96(poolAddress,poolAbi)
...
const amountIn = ethers.utils.parseUnits(
  ...
  const nonce2 = await web3.eth.getTransactionCount(WALLET_ADDRESS);
  const gasPrice = await web3.eth.getGasPrice();
  const gasLimit = 488000;
 //const gasLimit = 214176971744;
